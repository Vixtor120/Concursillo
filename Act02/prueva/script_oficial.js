...
function goToNextLevel() { // Función para ir al siguiente nivel
    window.location.href = 'lore2.html'; // Reemplaza con la URL real
}
    ...
    function iniciarCuentaAtras() {
        const intervalo = setInterval(() => {
            if (tiempoRestante <= 0) {
                ...
                showLosePopup(); // Mostrar el pop-up de derrota al finalizar el tiempo
            } else {
                contadorElemento.textContent = `Tiempo restante: ${tiempoRestante} segundos`;
                if (tiempoRestante === 20) {
                    warningImage.classList.remove('hidden4'); // Mostrar la imagen cuando queden 20 segundos
                }
